...
int semi_touch_hall_init_buff(struct sm_touch_dev *st_dev)
{
    ...
    if (NULL == dma_buff_virtual_addr)
    {
        ...
        dma_buff_virtual_addr = (unsigned char *)dma_alloc_coherent(&client->dev, DMA_BUFFER_LENGTH, &dma_buff_physical_addr, GFP_KERNEL);
...
int semi_touch_hall_exit(void)
{
    if (dma_buff_virtual_addr)
    {
        dma_free_coherent(&st_dev.client->dev, DMA_BUFFER_LENGTH, dma_buff_virtual_addr, dma_buff_physical_addr);
